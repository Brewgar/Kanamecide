...
| 6 | Confirmed CLI-assigned IDs before writing | `research.py experiments` / `handoffs` → the CLI allocated the five-digit ids **E-00014/E-00015** (NOT the four-digit forms I had drafted) plus HO-0014/15/16; I corrected all 30 forward references in my addendum to match | HIGH — caught and fixed before commit; this is why "verify with the CLI" mattered |
...
| 8 | Filed E-00014 and E-00015 PENDING, each with inputs, pre-registered output fields, and abort conditions | `research/experiments/E-00014-*.md` (8 abort conditions, 4 branches) and the E-00015 record (7 abort conditions, 4 branches) | HIGH — no field omitted; every branch maps to a named E-0013 outcome |
| 9 | Filed HO-0014 (discharge verification only, answer withheld) + HO-0015/HO-0016 | `research/handoffs/HO-001{4,5,6}-*.md` | HIGH |
| 10 | Corrected a dangling forward-reference I had introduced | validate flagged a dangling future-experiment id in my draft; I removed the id and left that contract CLI-assigned-at-filing | MEDIUM — self-caught by validate, fixed before commit; the fix line itself is worded to avoid re-introducing a dangling id |
